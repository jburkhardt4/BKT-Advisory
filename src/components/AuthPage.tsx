...
// Shared SSO button geometry — every provider button uses this identical box so
// padding, radius, and height stay unified. Only the surface colors vary below.
const SSO_BUTTON_BASE =
  "w-full flex items-center justify-center gap-3 px-4 py-2.5 shadow-sm rounded-lg text-sm font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed";
const SSO_SURFACE_LIGHT =
  "bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-300 text-slate-700";
// LinkedIn brand neutral. Hover darkens via brightness rather than a second hex
// so the flat-color and gradient surfaces below share one hover mechanism.
const SSO_SURFACE_LINKEDIN =
  "bg-[#F4F2EE] hover:brightness-[0.97] border border-slate-200 hover:border-slate-300 text-slate-700";
// Microsoft: vertical ramp from the LinkedIn neutral to a cool near-white. A
// `hover:bg-*` color would sit under the gradient and never show, hence brightness.
const SSO_SURFACE_MICROSOFT =
  "bg-[linear-gradient(to_bottom,#F4F2EE,#EAE2DF,#ECE8E7,#EBEBEB,#ECF0F1,#EEF3F6,#F1F6FA,#F4FAFD)] hover:brightness-[0.97] border border-slate-200 hover:border-slate-300 text-slate-700";
// GitHub brand surface: #24292F, lightened to #2C3238 on hover. The 1px border
// matches the light buttons' border so the rendered height is identical.
const SSO_SURFACE_GITHUB =
  "bg-[#24292F] hover:bg-[#2C3238] border border-[#24292F] hover:border-[#2C3238] text-white";
              ...
              <div className="space-y-3 mb-6">
                {ssoProviders.map((provider) => (
                  <button
                    ...
                    disabled={!!ssoLoading}
                    className={`${SSO_BUTTON_BASE} ${provider.surface}`}
                  >
